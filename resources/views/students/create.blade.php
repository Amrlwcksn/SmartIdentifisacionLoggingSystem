        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                ...
                <h2 class="text-3xl font-black text-slate-900 dark:text-white tracking-tight">
                    Tambah <span class="text-indigo-600 dark:text-indigo-500">Siswa Baru</span>
                </h2>
                <p class="text-sm text-slate-500 mt-1 font-medium">Lengkapi formulir di bawah untuk mendaftarkan siswa ke sistem.</p>
            </div>
    ...
    <div class="max-w-4xl">
        <div class="bg-white dark:bg-[#1e293b] rounded-[2rem] border border-slate-200 dark:border-slate-700/50 shadow-xl overflow-hidden transition-all duration-300">
            <div class="px-8 py-6 bg-slate-50 dark:bg-slate-800/30 border-b border-slate-200 dark:border-slate-700/50">
                <h3 class="text-sm font-black text-slate-900 dark:text-white uppercase tracking-wider">Formulir Pendaftaran Siswa</h3>
            </div>
            ...
            <form action="{{ route('students.store') }}" method="POST" class="p-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="class" id="class" required value="{{ old('class') }}"
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <select name="parent_id" id="parent_id" required
                                ...
                                <option value="" disabled selected class="text-slate-400 dark:text-slate-600">Pilih salah satu...</option>
                                @foreach($parents as $parent)
                                    <option value="{{ $parent->id }}" {{ old('parent_id') == $parent->id ? 'selected' : '' }}>{{ $parent->name }} ({{ $parent->phone }})</option>
                                @endforeach
                            </select>
                ...
                <div class="mt-12 flex items-center justify-end space-x-4">
                    <button type="reset" class="px-6 py-3 text-xs font-black text-slate-500 uppercase tracking-widest hover:text-white transition-all">Reset Form</button>
                    <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-4 rounded-2xl text-xs font-black uppercase tracking-widest transition-all shadow-lg shadow-indigo-600/20">Simpan Data Siswa</button>
                </div>
            </form>
        </div>
    </div>
