<x-app-layout>
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                ...
                <h2 class="text-3xl font-black text-slate-900 dark:text-white tracking-tight">
                    Edit <span class="text-indigo-600 dark:text-indigo-500">Wali Murid</span>
                </h2>
                <p class="text-sm text-slate-500 mt-1 font-medium">Perbarui informasi untuk: <strong>{{ $parent->name }}</strong></p>
            </div>
    ...
    <div class="max-w-4xl space-y-8">
        <div class="bg-white dark:bg-[#1e293b] rounded-[2rem] border border-slate-200 dark:border-slate-700/50 shadow-xl overflow-hidden transition-all duration-300">
            ...
            <form action="{{ route('parents.update', $parent->id) }}" method="POST" class="p-8">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" name="name" id="name" required value="{{ old('name', $parent->name) }}"
                        ...
                        <div>
                            ...
                            <input type="text" name="phone" id="phone" required value="{{ old('phone', $parent->phone) }}"
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" name="telegram_id" id="telegram_id" value="{{ old('telegram_id', $parent->telegram_id) }}"
                        ...
                        <div>
                            ...
                            <textarea name="address" id="address" rows="3"
                                ...
                                placeholder="Masukkan alamat tinggal...">{{ old('address', $parent->address) }}</textarea>
                ...
                <div class="mt-12 flex items-center justify-end space-x-4">
                    <button type="reset" class="px-6 py-3 text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest hover:text-slate-900 dark:hover:text-white transition-all">Batalkan</button>
                    <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-4 rounded-2xl text-xs font-black uppercase tracking-widest transition-all shadow-lg shadow-indigo-600/20">Simpan Perubahan</button>
                </div>
            </form>
        </div>
        ...
        <div class="bg-amber-50 dark:bg-amber-500/5 border border-amber-200 dark:border-amber-500/20 rounded-3xl p-8 flex items-start space-x-6 transition-all duration-300">
            ...
            <div>
                ...
                <ul class="text-[11px] text-slate-500 space-y-2 font-bold uppercase tracking-tight transition-all duration-300">
